...
app.post('/api/ai/rewrite-resume', aiLimiter, async (req, res) => {
  try {
    ...
    const messages = [
      {
        ...
        content:
          ...
          '不要整篇重写，不要输出空泛套话；优先改最值得改的原文句子。只有当用户补充确认或经历库素材中有真实经历、且原简历没有写进去时，才可以输出新增段落建议。' +
          '替换建议和新增段落建议都必须能说明它如何更贴合 JD、更具体、更简洁、更便于 HR/ATS 扫读。' +
          '如果原文定位不稳、事实依据不足、或改后只是同义替换，请不要强行给建议，改为放进 questionsForUser 或 overallNotes。' +
          '新增段落建议必须提供可在原简历中逐字定位的插入锚点 insertAfterText；无法定位插入位置时不要输出新增段落。' +
      ...
      },
      {
        ...
        content:
          ...
          '"insertSuggestions": [ { "section": "", "insertAfterText": "", "insertedText": "", "reason": "", "truthCheckRequired": true, "qualityScore": 4, "confidence": 0.8, "impact": "", "qualityChecks": { "jdFit": 4, "specificity": 4, "truthSafety": 5, "readability": 4 } } ], ' +
          ...
          '\ninsertSuggestions 是新增段落建议：只能使用【用户补充的真实信息】或原简历已有事实，不得编造。insertAfterText 必须从【原简历文本】逐字摘录一段现有原文，表示新增内容插在该段之后；insertedText 是要新增到 Word 的段落。没有清晰插入锚点或没有真实补充素材时返回空数组。' +
    ...
    if (data && Array.isArray(data.insertSuggestions)) {
      data.insertSuggestions = data.insertSuggestions.map(s => {
        const score = Math.max(1, Math.min(5, Number(s && s.qualityScore) || 3));
        const confidence = Math.max(0, Math.min(1, Number(s && s.confidence) || 0));
        const checks = (s && typeof s.qualityChecks === 'object' && s.qualityChecks) || {};
        return Object.assign({}, s, {
