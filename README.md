...
- **岗位链接抓取**：在岗位弹窗粘贴公开招聘页链接，可实时抓取页面中的岗位 JD 并填入表单；尊重 robots.txt，不绕过登录或验证码。
- **职位看板**：从企业官网招聘页或官方 ATS 抓取真实职位，自动用当前简历计算快速匹配度，并可一键保存到岗位库；已适配腾讯、百度、京东、美团、中信证券官网，以及 Greenhouse、Lever、Ashby，并内置“大厂 / 券商”来源预设。
- **AI 匹配分析**：基于真实 JD、简历文本和本地职业资料库分析匹配点、缺口、评分维度、引用依据和修改重点。
...
- 资料库：内置职业资料种子库（O*NET、BLS、ESCO、人社部公开入口等来源链接），可在 `server/lib/careerReferences.js` 中扩展
